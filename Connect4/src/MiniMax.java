...
// Contains the AI logic for determining next move
public class MiniMax 
{
	...
	// Board cache
	private static HashMap<int[], Integer> memoizedBrd = new HashMap<int[], Integer>();
	...
	int DoSearch( Position pos, int nPiece, int nDepth, Board pBoard, int nAlpha, int nBeta )
	{
		...
		if(memoizedBrd.containsKey(nMoves) && nDepth != 0)
			return memoizedBrd.get(nMoves).intValue();
		...
		if(nPiece == m_nSearchPiece)
		{
			...
			{
				// Save a copy of the board before placing pieces
				Board tempSave = pBoard.Clone();
				pBoard.PlacePiece(nMoveList[i*2], nMoveList[i*2+1], nPiece);
				// Save nValue before possible changes
				int tempVal = nValue;
			...
			// Memoize 
			memoizedBrd.put(nMoveList, nValue);
			return nValue;
	...
	public void GetMove( Position pos, int[][] BoardData, int nPiece )
	{
		...
		DoSearch( pos, nPiece, 0, brd, Integer.MIN_VALUE, Integer.MAX_VALUE );
	}
